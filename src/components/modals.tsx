...
function Field({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="border-b border-ink-100 py-2.5">
      ...
      <div className="mt-1 text-[17.5px] text-ink-800">{value}</div>
    </div>
  );
}
    ...
    <Dialog>
      ...
      >
        <div className="border-b border-ink-200/60 bg-ivory-100/50 px-5 py-4">
          <p className="mb-2 text-[15.5px] text-ink-400">주문 진행단계</p>
        ...
        <div className="px-5 py-4">
          <p className="text-[15px] text-ink-400">제작 요청사항</p>
          <p className="mt-1.5 rounded-md border border-ink-200 bg-ivory-100/60 px-3 py-2.5 text-[17px] leading-relaxed text-ink-700">
            {order.note}
          </p>
        </div>
  ...
  return (
    <Dialog>
      ...
      <DialogContent title={project.name} desc={`${project.id} · ${project.product} · 담당 ${project.owner}`}>
        <div className="border-b border-ink-200/60 bg-ivory-100/50 px-5 py-4">
          <p className="mb-2 text-[15.5px] text-ink-400">사업화 단계</p>
        ...
        <div className="grid gap-4 border-b border-ink-100 px-5 py-4 sm:grid-cols-2">
          <div>
            <div className="mb-1.5 flex items-center justify-between text-[15.5px]">
      ...
      >
        {done ? (
          <div className="px-5 py-10 text-center">
            <p className="text-[19px] font-medium text-ink-800">집행 건이 등록되었습니다.</p>
            <p className="mt-1 text-[17px] text-ink-400">
          ...
          <>
            <div className="space-y-3.5 px-5 py-4">
              <div>
                <label className="mb-1.5 block text-[15.5px] text-ink-500">회사 구분</label>
                <div className="grid grid-cols-2 gap-2">
                  {(["tailor", "corp"] as CompanyId[]).map((c) => (
                    <button
                      ...
                      className={cn(
                        "rounded-md border px-3 py-2 text-[17px] font-medium transition-colors",
                        company === c
                          ? c === "tailor"
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  <label className="mb-1.5 block text-[15.5px] text-ink-500">집행 항목</label>
                  <select
                    value={category}
                    onChange={(e) => setCategory(e.target.value)}
                    className="w-full rounded-md border border-ink-200 bg-white px-3 py-2 text-[17px] text-ink-800 outline-none focus:border-ink-400"
                  >
                    {categories.map((c) => (
                ...
                <div>
                  <label className="mb-1.5 block text-[15.5px] text-ink-500">지급 방식</label>
                  <select
                    value={method}
              ...
              <div>
                <label className="mb-1.5 block text-[15.5px] text-ink-500">집행 내용</label>
                <input
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
                  placeholder="예) Zegna Trofeo 600 네이비 12m 발주"
                  className="w-full rounded-md border border-ink-200 bg-white px-3 py-2 text-[17px] text-ink-800 outline-none placeholder:text-ink-300 focus:border-ink-400"
                />
              </div>

              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  <label className="mb-1.5 block text-[15.5px] text-ink-500">거래처</label>
                  <input
                    value={vendor}
                ...
                <div>
                  <label className="mb-1.5 block text-[15.5px] text-ink-500">집행 금액 (원)</label>
                  <input
                    value={amount}
                    onChange={(e) => setAmount(e.target.value.replace(/[^\d,]/g, ""))}
                    inputMode="numeric"
                    placeholder="0"
                    className="w-full rounded-md border border-ink-200 bg-white px-3 py-2 text-right text-[17px] text-ink-800 outline-none placeholder:text-ink-300 focus:border-ink-400 num"
                  />
                </div>
              </div>

              <p className="rounded-md bg-ivory-100 px-3 py-2 text-[15.5px] text-ink-400">
                등록 시 결재자는 박정열 대표로 지정되며, 상태는 <b className="text-ink-600">승인 대기</b>로 저장됩니다.
              </p>
            </div>
            <div className="flex justify-end gap-2 border-t border-ink-200/60 bg-ivory-100/40 px-5 py-3">
          ...
          {AI_PROJECTS.map((p) => (
            <li key={p.id} className="rounded-md px-3 py-2.5 hover:bg-ivory-100">
              <div className="flex items-center justify-between gap-3">
                <span className="text-[17.5px] font-medium text-ink-800">{p.name}</span>
              ...
              <div className="mt-1.5 flex items-center gap-3">
                ...
                <span className="text-[15.5px] text-ink-400">{p.phase}</span>
                <span className="ml-auto text-[15.5px] text-ink-400 num">{p.dueAt}</span>
              </div>
            </li>
          ))}
        </ul>
        <div className="border-t border-ink-200/60 px-5 py-3 text-[15.5px] text-ink-400">
