...
const AXIS = { fontSize: 15, fill: "#8a94a8" } as const;
const GRID = "#eceef2";
...
function TooltipBox({
  ...
  return (
    <div className="rounded-md border border-ink-200 bg-white px-3 py-2 shadow-panel">
      {label ? <p className="mb-1 text-[15.5px] font-medium text-ink-700">{label}</p> : null}
      {payload.map((p, i) => (
        <p key={i} className="flex items-center gap-2 text-[15.5px] text-ink-600">
          <span
      ...
      <LineChart data={data} margin={{ top: 8, right: 8, left: -6, bottom: 0 }}>
        ...
        {showLegend ? (
          <Legend
            ...
            iconType="circle"
            iconSize={7}
            wrapperStyle={{ fontSize: 15.5, color: "#5f6a80" }}
          />
        ) : null}
        {showTailor ? (
          <Line
  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      ...
      <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-[20.5px] font-semibold text-ink-800 num">{value}%</span>
        {label ? <span className="text-[13px] text-ink-400">{label}</span> : null}
      </div>
    </div>
  );
}
...
export function CategoryDonut({
  ...
  return (
    <div className="relative" style={{ height }}>
      ...
      {centerTop ? (
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-[15px] text-ink-400">{centerBottom}</span>
          <span className="text-[21.5px] font-semibold text-ink-800 num">{centerTop}</span>
        </div>
      ) : null}
    </div>
  );
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} margin={{ top: 8, right: 8, left: -8, bottom: 0 }} barGap={3}>
        <CartesianGrid stroke={GRID} vertical={false} />
        <XAxis dataKey="name" tick={{ ...AXIS, fontSize: 14 }} axisLine={false} tickLine={false} interval={0} />
        <YAxis
          tick={AXIS}
          axisLine={false}
          tickLine={false}
