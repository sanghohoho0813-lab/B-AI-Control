              ...
              <MCard key={u.id} onPress={() => go(u.href)}>
                <div className="p-3.5">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="ml-auto text-[15px] text-ink-400">{u.at}</span>
                  </div>
                  <div className="mt-2 flex items-start gap-2">
                    <div className="min-w-0 flex-1">
                      <p className="text-[19px] font-semibold leading-snug text-ink-800">
                        {u.title}
                      </p>
                      <p className="mt-1 text-[16px] leading-relaxed text-ink-400">{u.detail}</p>
                    </div>
        ...
        return (
          <MSection key={d} className="mt-5">
            <div className="mb-2 flex items-center gap-2 px-1">
              <h2 className="text-[19px] font-semibold tracking-tight text-ink-800 num">{d}</h2>
              {isToday ? (
                <span className="rounded-full bg-ink-800 px-2 py-[3px] text-[14.5px] font-medium leading-none text-white">
                  오늘
                </span>
              ) : null}
              <span className="ml-auto text-[15.5px] text-ink-400">{dayItems.length}건</span>
            </div>
            <MTimeline>
    ...
    <>
      ...
      <MCard className="border-0 bg-ink-800">
        <div className="p-4">
          ...
          <p className="mt-1.5 text-[36px] font-semibold leading-none tracking-tight text-white num">
            {won(available)}
          </p>
          <div className="mt-3.5 grid grid-cols-3 gap-2 border-t border-white/10 pt-3 text-center">
            <div>
              <p className="text-[14.5px] text-white/50">계획</p>
              <p className="mt-1 text-[17.5px] font-medium text-white num">{wonShort(planned)}</p>
            </div>
            <div>
              <p className="text-[14.5px] text-white/50">집행</p>
              <p className="mt-1 text-[17.5px] font-medium text-white num">{wonShort(executed)}</p>
            </div>
            <div>
              <p className="text-[14.5px] text-white/50">집행률</p>
              <p className="mt-1 text-[17.5px] font-medium text-white num">
              ...
              return (
                <MCard key={c}>
                  <div className="p-3.5">
                    <span
                      className={cn(
                        "block break-keep text-[15.5px] font-medium leading-snug",
                        c === "tailor" ? "text-tailor-700" : "text-corp-700",
                      )}
                    >
                      {COMPANIES[c].shortName}
                    </span>
                    <p className="mt-2 text-[31px] font-semibold leading-none text-ink-900 num">
                      {pct(e, p)}%
                    </p>
                    <div className="mt-2">
                      <MProgress value={pct(e, p)} tone={c} />
                    </div>
                    <p className="mt-1.5 text-[15px] text-ink-400 num">
            ...
            {lines.map((b) => (
              <div key={b.id} className="px-3.5 py-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="flex-1 truncate text-[17.5px] font-medium text-ink-800">
                    {b.category}
                  </span>
...
const MORE_GROUPS: {
  ...
  },
  {
    ...
    items: [
      ...
      { label: "상품 라인업", href: "/tailor/products" },
    ...
    <>
      <MCard>
        <div className="flex items-center gap-3 p-4">
          <span className="flex h-11 w-11 items-center justify-center rounded-full bg-ink-800 text-[20.5px] font-semibold text-white">
            박
          </span>
          <div className="min-w-0 flex-1">
            <p className="text-[20.5px] font-semibold text-ink-800">박정열 대표</p>
            <p className="mt-0.5 break-keep text-[16px] leading-snug text-ink-400">
              비앤테일러샵 · AI 소프트웨어 법인
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-px border-t border-ink-100 bg-ink-100">
          ...
          >
            <p className="text-[15px] text-ink-400">관리 법인</p>
            <p className="mt-0.5 text-[20.5px] font-semibold text-ink-800 num">2개</p>
          </button>
          <button
            onClick={() => go("/notifications")}
            className="bg-white px-3.5 py-3 text-left active:bg-ivory-100"
          >
            <p className="text-[15px] text-ink-400">읽지 않은 알림</p>
            <p className="mt-0.5 text-[20.5px] font-semibold text-ink-800 num">4개</p>
          </button>
        </div>
      </MCard>
      ...
      <p className="mt-6 pb-2 text-center text-[15px] text-ink-300">
