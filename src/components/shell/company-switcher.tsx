...
export function CompanySwitcher({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className="flex items-center gap-2">
      {!compact ? (
        <span className="mr-0.5 text-[16px] font-medium text-ink-400">Company</span>
      ) : null}
        ...
        {OPTIONS.map((opt) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex items-center gap-1.5 rounded px-2.5 py-1.5 text-[17px] font-medium transition-colors",
                active
                  ? opt.value === "tailor"
            ...
            >
              <span
                className={cn(
                  "flex h-[17px] min-w-[17px] items-center justify-center rounded-[3px] px-1 text-[12px] font-bold",
                  active
