        ...
        {TYPES.filter((t) => t !== "전체").map((t) => {
          ...
          return (
            <Card key={t}>
              ...
              >
                <p className="text-[16px] text-ink-500">{t}</p>
                <p className="mt-1.5 text-[25px] font-semibold text-ink-900 num">
                  {cnt}
          ...
          <Card key={d.id}>
            <div className="p-3.5">
              <div className="flex items-start justify-between gap-2">
                <span className="text-[17px] font-semibold text-ink-800">{d.name}</span>
                <StatusBadge status={d.status} />
              </div>
              <p className="mt-1.5 text-[20.5px] font-semibold text-corp-700 num">{d.scale}</p>
              <p className="mt-1 text-[15px] leading-relaxed text-ink-400">{d.note}</p>
              <p className="mt-2 border-t border-ink-100 pt-2 text-[15px] text-ink-400">
