          ...
          <div className="mt-4 grid grid-cols-7 gap-2">
            {ORDER_FUNNEL.map((f) => (
              <div key={f.stage} className="rounded-md border border-ink-200/70 bg-ivory-100/50 p-2.5">
                <p className="text-[15px] text-ink-400">{f.stage}</p>
                ...
                <p className="mt-0.5 text-[14.5px] text-ink-400 num">{wonShort(f.amount)}</p>
              </div>
            ))}
          </div>
        </div>
              ...
              {risky.map((j) => (
                <li key={j.id} className="border-b border-ink-100 px-4 py-3 last:border-b-0">
                  <div className="flex items-center justify-between gap-2">
                    <span className="flex items-center gap-1.5 text-[17px] font-medium text-ink-800">
                      <AlertTriangle
                        className={cn(
                          "h-3.5 w-3.5",
                ...
                <li key={o.id} className="border-b border-ink-100 last:border-b-0">
                  <OrderDetailModal order={o}>
                    <button className="flex w-full items-center gap-3 px-4 py-2.5 text-left transition-colors hover:bg-ivory-100">
                      ...
                      <span className="w-[74px] shrink-0 text-right text-[15.5px] text-ink-500 num">
                        {o.dueAt.slice(5)}
                      </span>
                    </button>
                  </OrderDetailModal>
          ...
          <Card className="h-full">
            ...
            <ul>
              {repurchase.map((c) => (
                ...
                >
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-tailor-50 text-[15px] font-semibold text-tailor-700">
                    {c.name.slice(0, 1)}
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="flex items-center gap-1.5">
                    ...
                    <span className="block truncate text-[15px] text-ink-400">{c.nextAction}</span>
                  </span>
                  <span className="shrink-0 text-right">
                    <span className="block text-[17px] font-semibold text-ink-800 num">
                      {c.repurchaseScore}
                    </span>
                    <span className="block text-[14px] text-ink-400">추천도</span>
                  </span>
                </li>
              ))}
            </ul>
            ...
            .map((s) => (
              <div key={s.id} className="p-3.5">
                <div className="flex items-center gap-2">
                  <span className="text-[15.5px] text-ink-400 num">{s.date.slice(5)}</span>
                  <Badge className="border-tailor-200 bg-tailor-50 text-tailor-700">{s.kind}</Badge>
                  <span className="ml-auto text-[15.5px] text-ink-400 num">{s.time}</span>
                </div>
                <p className="mt-1.5 text-[17px] font-medium text-ink-800">{s.title}</p>
